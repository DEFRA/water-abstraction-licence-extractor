...
public class WqFormPageSplitTests(ITestOutputHelper testOutputHelper)
{
    ...
    /// </summary>
    private static readonly Regex MoveOperatorRegex = new(
        @"(?<tlval>[\d.\-]+)\s+TL\b" +
    ...
    /// advances a line at a time from there, rather than resetting on every line. Previously this
    /// tool declined the whole file rather than attempt this; walking the chain instead lets the
    /// relocated content (which starts a brand new BT on a brand new page, with none of this
    /// TD-chain history to inherit) carry a synthetic Tm anchored at exactly the right spot.
    /// </summary>
    /// <summary>
    ...
    private static bool TryComputeAbsolutePositionAtCutPoint(
    ...
    /// data instead of baked into a string immediately (unlike the initial cut in
    /// <see cref="TrySplitAndInsert"/>, a cascade cut's final Y depends on where in the flow it
    /// lands, not a single fixed constant).
    /// </summary>
    private readonly record struct AnchorMatrix(string A, string B, string C, string D, double X);
    ...
    /// was authored for - the same fork <see cref="TrySplitAndInsert"/>'s own initial cut already
    /// makes for <c>movedContent</c>, generalized to also cover the cascade's "head" fragments
    /// (which start at a page's own true beginning, so they're self-contained already and need no
    /// injected font at all).
    /// </summary>
    private enum FragmentWrapMode
    {
    ...
    /// bakes the caller-supplied final Y directly (mirroring the initial cut's own synthetic-Tm
    /// handling in <see cref="TrySplitAndInsert"/>) rather than relying on the generic per-Tm
    ...
    /// </summary>
    private static string BuildShiftedFragment(
    ...
    /// and reopens synthetic equivalents on the other side - the same rebalancing technique
    /// <see cref="TrySplitAndInsert"/>'s own MarkedContentPerFragment cut and footer-extraction
    /// steps already use inline, factored out so the reflow cascade's own cuts (both its main
    /// content cut and its own footer extraction) can reuse it instead of a third copy.
    /// </summary>
    private static (string ClosedHead, string ReopenedTail) RebalanceMarkedContentAt(
        string headUpToSplit, string tailFromSplit)
    {
        var contentForDepthScan = WqFormSpliceAndOverlayTests.StringRunRegex.Replace(headUpToSplit, "");
        var openSpanCount = 0;
    ...
    /// zero across the whole thing. Used by the reflow cascade in <see cref="TrySplitAndInsert"/>
    /// as a final safety check on every fragment it's about to write - <see
    /// cref="RebalanceMarkedContentAt"/> is designed to always produce balanced halves, but a
    /// handful of real files have marked-content/graphics-state nesting its depth-counting
    /// doesn't model correctly, and an imbalanced fragment is a real, visible defect (a
    /// "Mismatched EMC operator" or "Restoring state" error) rather than something safe to ship
    /// and hope looks fine.
    /// </summary>
    private static bool IsMarkedContentBalanced(string fragment)
    {
        ...
        var stripped = WqFormSpliceAndOverlayTests.StringRunRegex.Replace(fragment, "");
        var bdcBalance = 0;
        var stateBalance = 0;
        var textBalance = 0;
