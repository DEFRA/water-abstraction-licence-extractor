...
public static class PdfContentReaderExtract
{
    ...
    private static List<HeaderContentSection> ExtractHeadersAndContent(List<string> allText)
    {
        var sections = new List<HeaderContentSection>();

        try
        {
            var currentSection = new HeaderContentSection();
            ...
            foreach (var line in allText)
            {
                ...
                if (isHeader)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(currentSection.Header) || currentSection.Content.Any())
                    {
                        sections.Add(currentSection);
                    }

                    // Start new section
                    currentSection = new HeaderContentSection
                    {
                        Header = trimmedLine,
                ...
                {
                    // No header found yet, treat as content without header
                    if (string.IsNullOrWhiteSpace(currentSection.Header))
                    {
                        currentSection.Header = "[Content without header]";
                    }
                    currentSection.Content.Add(trimmedLine);
                }
            }
        ...
        {
            Console.WriteLine($"Error extracting headers and content: {ex.Message}");
        }
