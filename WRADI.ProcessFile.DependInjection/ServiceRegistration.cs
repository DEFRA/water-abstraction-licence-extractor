...
namespace WRADI.ProcessFile.DependInjection;
...
public static class ServiceRegistration
{
        ...
        IConfiguration configuration)
    {
        ...
        services.AddSingleton<IOrchestrateFileProcess, OrchestrateFileProcessService>();
        services.AddHttpClient<IOrchestratorService, OrchestratorService>((sp, client) =>
        {
            var settings = sp.GetRequiredService<AppSettings>();
            client.BaseAddress = new Uri(settings.ApiBaseUrl);
        });
        
        services.AddSingleton<IScrapeFileService, SingleFileProcessService>();
        return services;
    }
}