...
public class TesseractAndAzureAiVisionOcrPdfTests
{
    ...
    public async Task WhenComplicatedFile_ThenGetPurposesPointsAbstractionLimitsCorrectly()
    {
        ...
        Assert.Equal("A", licence.AbstractionLimits.Individual[0].Limits[0].Points![0].Id);
        Assert.True(licence.AbstractionLimits.Individual[0].Limits[0].Points![0].IsImplicit);
        Assert.Single(licence.AbstractionLimits.Individual[0].Limits[0].Purposes!);
        Assert.Equal("(1)", licence.AbstractionLimits.Individual[0].Limits[0].Purposes![0].Id);
        Assert.False(licence.AbstractionLimits.Individual[0].Limits[0].Purposes![0].IsImplicit);
        ...
        Assert.Equal("A", licence.AbstractionLimits.Individual[1].Limits[0].Points![0].Id);
        ...
        Assert.Equal("(2)", licence.AbstractionLimits.Individual[1].Limits[0].Purposes![0].Id);
        ...
        Assert.Single(licence.AbstractionLimits.Aggregates[0].Limits[0].Points!);
        Assert.Equal("A", licence.AbstractionLimits.Aggregates[0].Limits[0].Points![0].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[0].Points![0].IsImplicit);
        Assert.Equal(2, licence.AbstractionLimits.Aggregates[0].Limits[0].Purposes!.Length);
        Assert.Equal("(1)", licence.AbstractionLimits.Aggregates[0].Limits[0].Purposes![0].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[0].Purposes![0].IsImplicit);
        Assert.Equal("(2)", licence.AbstractionLimits.Aggregates[0].Limits[0].Purposes![1].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[0].Purposes![1].IsImplicit);
        ...
        Assert.Single(licence.AbstractionLimits.Aggregates[0].Limits[1].Points!);
        Assert.Equal("A", licence.AbstractionLimits.Aggregates[0].Limits[1].Points![0].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[1].Points![0].IsImplicit);
        Assert.Equal(2, licence.AbstractionLimits.Aggregates[0].Limits[1].Purposes!.Length);
        Assert.Equal("(1)", licence.AbstractionLimits.Aggregates[0].Limits[1].Purposes![0].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[1].Purposes![0].IsImplicit);
        Assert.Equal("(2)", licence.AbstractionLimits.Aggregates[0].Limits[1].Purposes![1].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[1].Purposes![1].IsImplicit);
        ...
        Assert.Single(licence.AbstractionLimits.Aggregates[0].Limits[2].Points!);
        Assert.Equal("A", licence.AbstractionLimits.Aggregates[0].Limits[2].Points![0].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[2].Points![0].IsImplicit);
        Assert.Equal(2, licence.AbstractionLimits.Aggregates[0].Limits[2].Purposes!.Length);
        Assert.Equal("(1)", licence.AbstractionLimits.Aggregates[0].Limits[2].Purposes![0].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[2].Purposes![0].IsImplicit);
        Assert.Equal("(2)", licence.AbstractionLimits.Aggregates[0].Limits[2].Purposes![1].Id);
        Assert.True(licence.AbstractionLimits.Aggregates[0].Limits[2].Purposes![1].IsImplicit);
    }
    ...
    public async Task WhenComplicatedFile2_ThenGetPurposesPointsAbstractionLimitsCorrectly()
    {
        ...
        Assert.Single(agg.Limits[0].Points!);
        Assert.Equal("(1)", agg.Limits[0].Points![0].Id);
        Assert.False(agg.Limits[0].Points![0].IsImplicit);
        Assert.Equal(2, agg.Limits[0].Purposes!.Length);
        Assert.Equal("(a)", agg.Limits[0].Purposes![0].Id);
        Assert.True(agg.Limits[0].Purposes![0].IsImplicit);
        Assert.Equal("(b)", agg.Limits[0].Purposes![1].Id);
        Assert.True(agg.Limits[0].Purposes![1].IsImplicit);
        ...
        Assert.Equal(LimitPeriodType.PerSecond, agg.Limits[2].PeriodType);        
        Assert.Single(agg.Limits[0].Points!);
        Assert.Equal("(2)", agg.Limits[0].Points![0].Id);
        Assert.False(agg.Limits[0].Points![0].IsImplicit);
        Assert.Equal(2, agg.Limits[0].Purposes!.Length);
        Assert.Equal("(a)", agg.Limits[0].Purposes![0].Id);
        Assert.True(agg.Limits[0].Purposes![0].IsImplicit);
        ...
        Assert.Equal(2, agg.Limits[0].Points!.Length);
        Assert.Equal("(1)", agg.Limits[0].Points![0].Id);
        Assert.True(agg.Limits[0].Points![0].IsImplicit);
        Assert.Single(agg.Limits[0].Purposes!);
        Assert.Equal("(2)", agg.Limits[0].Points![1].Id);
        Assert.True(agg.Limits[0].Points![1].IsImplicit);
        Assert.Single(agg.Limits[0].Purposes!);        
        Assert.Equal("(a)", agg.Limits[0].Purposes![0].Id);
